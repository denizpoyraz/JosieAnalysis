...
#######################################################################################################################
## calculate beta values from 0910 and 9602 data
...
for j in range(len(simlist)):
    ...
    ### 1 convention for smoothing,  gaussian with std 1 in window 4

    dft[j]['I_OPM_jma_sm'] = dft[j].I_OPM_jma.rolling(window=4, win_type='gaussian', center=True).mean(std=1)
    dft[j]['IM_sm'] = dft[j].IM.rolling(window=4, win_type='gaussian', center=True).mean(std=1)
    ...
    for i in range(1, size - 1):
        ...
        Islow[i] = beta * dft[j].at[i, 'I_OPM_jma_sm']
        Islow_conv[i + 1] = Islow[i] - (Islow[i] - Islow_conv[i]) * Xs

        Ifast[i + 1] = af * (dft[j].at[i + 1, 'IM_sm'] - Islow_conv[i + 1])
        Ifastminib0[i + 1] = af * (dft[j].at[i + 1, 'IM_sm'] - Islow_conv[i + 1] - dft[j].at[i + 1, 'iB0'] )
    ...
    dft[j]['Ifast_minib0_smb8'] = dft[j].Ifast_minib0.rolling(window=4, win_type='gaussian', center=True).mean(std=1)
    ...
    dft[j]['Ifast_deconv_sm8'] = dft[j].Ifast_deconv.rolling(window=4, win_type='gaussian', center=True).mean(std=1)
    dft[j]['Ifast_minib0_deconv_sm8'] = dft[j].Ifast_minib0_deconv.rolling(window=4, win_type='gaussian', center=True).mean(std=1)
...
df_dc.to_csv("/home/poyraden/Analysis/JOSIEfiles/Proccessed/Josie2009_deconv_smoothfirst.csv")
